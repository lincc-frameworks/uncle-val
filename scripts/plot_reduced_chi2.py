#!/usr/bin/env python
"""Reduced-chi2 distributions of constant light curves for a trained model.

Renders the four-histogram per-band figure (full / random-subset light curves,
reported / model-corrected uncertainties) on shared bins. Uncertainties are
corrected by the model's error-scaling factor only (no signal correction).
"""
...
from uncle_val.pipelines.plotting import make_chi2_distribution_plot
...
def main():
    """Parse command-line arguments and render the reduced-chi2 figure."""
    ...
    output = args.output or args.model_dir / "plots" / f"model_{args.split}" / "reduced_chi2.pdf"
